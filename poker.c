...
//helper method for sorting cards
void swapC(card *c1, card *c2)
{
	card temp;
	memcpy(&temp, c1, sizeof(card));
	memcpy(c1, c2, sizeof(card));
	memcpy(c2, &temp, sizeof(card));
}

//helper method for sorting players
void swapP(player *p1, player *p2)
{
	player temp = *p1;
	*p1 = *p2;
	*p2 = temp;
...
//helper method for sorting hands
void swapH(hand *h1, hand *h2)
{
	hand temp = *h1;
	*h1 = *h2;
	*h2 = temp;
	...
	//while(fgets(game1, 32, fp)!=NULL) {
		...
		for(test=0; test<21; test++)
		{
			...
			for(testy=0; testy<5; testy++){
				printf("%u ", hands1[test].h[testy].rank);
			}
			printf("\n");
		}
